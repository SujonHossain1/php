...
<body>
	...
	<tr><td>
	<img src="'.$logo.'" alt="'.$alt.'" width="150px">
	</td>
	<td>
	<h1>QUEEN DOHS</h1><hr>
	...
	$query = "SELECT *,DATE(date) date FROM orders_info,order_products,products WHERE orders_info.order_id = order_products.order_id and order_products.product_id=products.product_id and orders_info.order_id=".$_GET['order_id'];
	$result = mysqli_query($con, $query) or die (mysqli_error($con));
	$serial = 1;
	while ($row = mysqli_fetch_assoc($result)) {
		$html .= '<tr>
		<td style="border-right:2px solid #ccc;">'.$serial.'</td>
		<td style="border-right:2px solid #ccc;"><img style="width: 50px;" src="'.$row['product_image'].'"</td>
	...
	$query = 'SELECT * from orders_info where order_id ='.$_GET['order_id']; 
	$result = mysqli_query($con, $query) or die (mysqli_error($con));
