...
(function() {
    ...
    var c = document,
        ...
        k = [],
        l = !1;
    "complete" === c.readyState ? a() : (c.addEventListener("DOMContentLoaded", a), e.addEventListener("load", a));
    ...
    String.prototype.endsWith || (String.prototype.endsWith =
        function(b, f) {
            if (void 0 === f || f > this.length) f = this.length;
            return this.substring(f - b.length, f) === b
        });
    e.$ = function(b, f) {
        if (b instanceof Function) return l ? b() : k.push(b), c;
        if (b instanceof NodeList) return new JQLite(h.call(b));
        if (b instanceof Node && 1 === b.nodeType) return new JQLite([b]);
        var a;
        if (a = "string" === typeof b) a = b.trim(), a = "<" === a[0] && ">" === a[a.length - 1] && 3 <= a.length;
        if (a) {
            b = b.trim();
            if (a = b.match(/^<([\w-]+)\s*\/?>(?:<\/\1>|)$/)) {
                f = f || {};
                ...
                b = [b]
            } else b.startsWith("<!DOCTYPE") || b.startsWith("<!doctype") ? b = [(new DOMParser).parseFromString(b, "text/html").documentElement] : (f = c.createElement("div"), f.innerHTML = b, b = h.call(f.childNodes));
            return new JQLite(b)
        }
        if ("string" === typeof b) return new JQLite(h.call(c.querySelectorAll(b)));
        if (b instanceof Document) return new JQLite([b.documentElement])
    };
    e.JQLite = function(b) {
        this.elements = b;
        return this
    ...
    g.prevSiblings = function() {
        for (var b = [], f = this.get(0); f = f.previousElementSibling;) b.push(f);
        return new JQLite(b)
    };
    g.nextSiblings = function() {
        for (var b = [], f = this.get(0); f = f.nextElementSibling;) b.push(f);
        return new JQLite(b)
    };
    ...
    g.attr = function(b, a) {
        if ("undefined" !== typeof a) return this.elements.forEach(function(f) {
            f.setAttribute(b,
                a)
        }), this;
    ...
    g.removeClass = function(b) {
        this.elements.forEach(function(a) {
            if (a.classList) a.classList.remove(b);
                ...
                if ($(a).hasClass(b)) {
                    for (; 0 <= f.indexOf(" " + b + " ");) f = f.replace(" " + b + " ", " ");
    ...
    g.val = function(b) {
        if (void 0 !== b) this.each(function() {
            this.value = b
        });
        else {
            var a = this.get(0);
    ...
    g.parents = function(b) {
        ...
        for (var a = $(this.get(0).parentElement); a.size() && !a.is(b);) a = a.parent();
        return a
    };
    g.find = function(b) {
        var a = [],
    ...
    g.is = function(b) {
        ...
        this.elements.forEach(function(e) {
            ...
            if (b.nodeType) f = e === b;
                ...
                for (var d = "string" === typeof b ? c.querySelectorAll(b) : [b], k = d.length; k--;) d[k] === e && (f = !0);
            a = f && a
        });
        return a
    };
    ...
    g.trigger = function(b) {
        try {
            ...
            a = c.createEvent("Event"), a.initCustomEvent(b, !0, !0)
        }
        this.elements.forEach(function(b) {
            b.dispatchEvent(a)
        })
    ...
    g.css = function(b, a) {
        if (b && "string" !== typeof b)
            for (var c in b) this.elements.forEach(function(a) {
                a.style[c] = b[c]
            });
        else {
            if ("undefined" === typeof a) return this.elements[0].style.getPropertyValue(b);
            this.elements.forEach(function(c) {
                c.style[b] = a
            });
            return this
    ...
    var q = function(a) {
        ...
        if ("string" !== typeof c && !(e.FormData && c instanceof FormData)) {
            k = [];
            for (var g in c) "GET" === b ? d = updateURLVar(d, g, c[g]) : k.push(g + "=" + encodeURIComponent(c[g]));
            c = k.join("&")
        }
        ...
        h.onreadystatechange = function() {
            if (h.readyState === XMLHttpRequest.DONE) {
                if (200 === h.status) {
                    ...
                    a.success(b)
                } else a.error(h, h.status, h.response);
                a.complete()
            }
        };
        h.open(b, d, !0);
    ...
    switch (a) {
        ...
        case "google-plus":
            window.open("https://plus.google.com/share?url=" +
                d, "sharer", "toolbar=0,status=0,width=660,height=445");
            break;
        case "pinterest":
...
$(function() {
    ...
    var d = $(".cart-toggler");
    d.on("click", function() {
        d.is(".loaded") || (cart.reload(), d.addClass("loaded"))
    })
});
var cart = {
        add: function(a, d) {
        ...
        update: function(a, d) {
            ...
            $.ajax({
                ...
                success: function(a) {
                    "checkout/cart" == getURLVar("route") || "checkout/checkout" == getURLVar("route") ? location = "checkout/cart" : ($(".cart-toggler .label").html(a.total), $(".cart-toggler .value").text(a.count), $(".cart-toggler").attr("title", a.total), c.reload())
                }
            })
        },
        remove: function(a, d) {
        ...
        add: function(a) {
            $.ajax({
                ...
                success: function(a) {
                    ...
                    a.info && (c = $(".container.alert-container"), 0 == c.size() && (c = $('<div class="container alert-container"></div>').insertAfter($(".after-header"))),
            ...
            $.ajax({
                ...
                success: function(a) {
                    ...
                    if (a.success) {
                        ...
                        0 == c.size() && (c = $('<div class="container alert-container"></div>'), $(".after-header").after(c));
                        c.append('<div class="alert alert-success"><i class="fa fa-check-circle"></i> ' + a.success + '<button type="button" class="close" data-dismiss="alert">&times;</button></div>');
...
$(document).delegate(".agree", "click", function(a) {
    ...
    $.ajax({
        ...
        success: function(a) {
            ...
            html += '        <h4 class="modal-title">' + $(d).text() +
                "</h4>";
            html += "      </div>";
...
$.fn.autocomplete = function(a) {
    return this.each(function() {
        ...
        c.response = function(a) {
            var e = "";
            if (a.length) {
                for (i = 0; i < a.length; i++) c.items[a[i].value] = a[i];
                for (i = 0; i < a.length; i++) a[i].category || (e += '<li data-value="' + a[i].value + '"><a href="#">' + a[i].label + "</a></li>");
                var l = [];
                for (i = 0; i < a.length; i++) a[i].category && (l[a[i].category] || (l[a[i].category] = [], l[a[i].category].name = a[i].category, l[a[i].category].item = []), l[a[i].category].item.push(a[i]));
                for (i in l)
                    for (e += '<li class="dropdown-header">' + l[i].name + "</li>", j = 0; j < l[i].item.length; j++) e += '<li data-value="' + l[i].item[j].value + '"><a href="#">&nbsp;&nbsp;&nbsp;' + l[i].item[j].label + "</a></li>"
            }
            e ? c.show() : c.hide();
            d.siblings("ul.dropdown-menu").html(e)
        };
        d.after('<ul class="dropdown-menu"></ul>');
        d.siblings("ul.dropdown-menu").delegate("a",
...
_t.activate = function(a) {
    this.active && (this.active.removeClass("active"), $("#" + this.active.attr("data-tab")).hide());
...
(function() {
    ...
    var c = [],
        e = function() {
            for (var e = [], l = 0; l < c.length; l++) d(c[l]) && (a(c[l]), e.push(c[l]));
            ...
            a ? $.ajax({
                ...
                success: function(a) {
                    ...
                    a.forEach(function(a) {
                        e = "remainder_cnt" == a.type ? e + ('<li class="search-item remainder-count"><a href="' + a.href + '">' + a.label + "</a></li>") : e + ('<li class="search-item"><a href="' + a.href + '"><div class="image"><img src="' + a.thumb + '"></div><div class="name">' + a.label + '</div><div class="price">' +
                            a.price + "</div></a></li>")
                    });
                    e ? c.show() : c.hide();
...
$(function() {
    ...
    function c(a) {
        var b = a.position,
            c = '<a href="' + a.url +
            '"><img src="' + a.image + '" alt="' + a.title + '" class="img-responsive"></a>';
...
/*    window.addEventListener("scroll", function() {
        165 < e.scrollTop() ? k.addClass("on-scroll") :
            k.removeClass("on-scroll")
    });*/
...
document.querySelector('.search-box').addEventListener('click', function () {
    const search_box_from = document.querySelector('.search-box-form');
    search_box_from.classList.toggle("search-box-form-show");
})