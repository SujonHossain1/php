                ...
                for($i=1; $i<=$countID; $i++)
                  {
                    ...
                    $query = "INSERT INTO `transaction_details`
                               (`ID`, `TRANS_D_ID`, `PRODUCTS`, `QTY`, `PRICE`, `EMPLOYEE`, `ROLE`,`sub_cat_id`,`cat_id`)
                               VALUES (Null, '{$today}', '".$_POST['name'][$i-1]."', '".$_POST['quantity'][$i-1]."', '".$_POST['price'][$i-1]."', '{$emp}', '{$rol}','".$_POST['sub_id'][$i-1]."','".$_POST['CATEGORY_ID'][$i-1]."')";

                    mysqli_query($db,$query)or die (mysqli_error($db));

                    }
                    $query111 = "INSERT INTO `transaction`
                               (`TRANS_ID`, `CUST_ID`, `NUMOFITEMS`, `SUBTOTAL`, `GRANDTOTAL`, `CASH`, `DATE`, `TRANS_D_ID`,`CUSTOMER_TYPE`,`REMARK`,`employee`)
                               VALUES (Null,'{$customer}','{$countID}','{$subtotal}','{$total}','{$cash}','{$date}','{$today}','{$customerType}','{$remark}','{$emp}')";
                    mysqli_query($db,$query111)or die (mysqli_error($db));
                    $due = $total - $cash;
                    $query2 = "INSERT INTO due VALUES(Null,'{$today}',$due)";
                    mysqli_query($db,$query2) or die(mysqli_error($db));
                    unset($_SESSION['pointofsale']);
                    header('Location: invoice.php?id='.$today);
                break;
              }
                    unset($_SESSION['pointofsale']);
               ?>
